...
namespace fastsim_multi
{
  ...
  public:
    ...
    // Structure created to get the pixel as well as the distance of the object
    // found by check_inter_is
    ...
    Return_check_t check_inter_is_dist(float x1, float y1, float x2,float y2) const;
    ...
    void add_robot_obstacle(Robot* robot);
    void clean_robot_obstacle(Robot* robot);

    void add_robot(Robot* robot)
    {
      _robots.push_back(robot);
      // add_robot_obstacle(robot->get_pos().x(), robot->get_pos().y(), robot->get_radius(), robot->get_bb().x, robot->get_bb().y, robot->get_bb().w, robot->get_bb().h);
      add_robot_obstacle(robot);
    }

    void remove_robot(int index)
    {
      // clean_robot_obstacle(_robots[index]->get_pos().x(), _robots[index]->get_pos().y(), _robots[index]->get_radius(), _robots[index]->get_bb().x, _robots[index]->get_bb().y, _robots[index]->get_bb().w, _robots[index]->get_bb().h);
      clean_robot_obstacle(_robots[index]);
  ...
  protected:
    ...
    boost::container::vector<Robot*> _robots;
    // 
    int _w, _h;
  ...
  struct ClosestObject_f
  {
    ClosestObject_f(float x, float y) : _x(x), _y(y) {}
    bool operator()(float x1, float y1, float x2, float y2)
    {
      float xDist1 = x1 - _x;
      float yDist1 = y1 - _y;
      float xDist2 = x2 - _x;
      float yDist2 = y2 - _y;
      float d1 = sqrtf(xDist1 * xDist1 + yDist1 * yDist1);
      float d2 = sqrtf(xDist2 * xDist2 + yDist2 * yDist2);
      return d1 < d2;
    }
    float _x, _y;
  };
